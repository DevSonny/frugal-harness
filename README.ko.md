...
**Antigravity 권장 모델:** 기본은 Sonnet과 Opus. 가벼운 작업이나 비용 절감이 필요하면 `Gemini 3.1 Pro (Low)` 또는 `Gemini 3.1 Pro (High)` 사용을 권장. Claude 쿼터나 Gemini 쿼터 중 한쪽이 소진됐을 때 다른 쪽으로 수동 지시하면 잘 동작한다 (예: "이 작업은 Gemini 3.1 Pro로 해줘"). `--model` 플래그로 직접 지정하면 됨.
