...
| `/exec` | 구현 | Codex |
| `/review` | 코드 리뷰 | Codex |
| `/docs` | 문서 작성/수정 | Antigravity (→ Codex 소진 시 → Claude 최후 수단) |
| `/ship` | 검증, 커밋, 푸시 | Codex |
...
문서 작업은 Antigravity가 1순위입니다. Antigravity가 실패하거나 쿼터가 소진되면 Codex가 맡고, 마지막 fallback으로 Claude가 문서를 직접 편집할 수 있습니다.
