...
| 에이전트 | 요금제 | 모델 | 역할 |
|---|---|---|---|
| **Claude Code** | Claude Pro $20/월 | `opusplan` | **계획 (Opus)** + 오케스트레이션 (Sonnet) |
| **Codex CLI** | ChatGPT Plus $20/월 | `gpt-5.5` | 구현, 리뷰, 커밋 & 푸시 |
| **Gemini CLI** | 무료 (1,000 req/일) | `gemini-2.5-flash-lite` | 문서 전담 — README, 변경 로그, 인라인 주석 |

**합계: 월 $40** — $100짜리 요금제 필요 없습니다.
Fallback: Codex나 Gemini 할당량이 소진되면 Claude가 임시 대체합니다.

세 모델 모두 설치 시 자동 고정됩니다 — `/model` 수동 설정 불필요.
Claude Code 는 기본값으로 `opusplan` 이 설정됩니다. Plan 모드에서는 Opus, 나머지 오케스트레이션에는 Sonnet 이 동작합니다. PreToolUse 훅 (아래 참고) 이 Sonnet 의 코드 직접 편집을 막아 Claude 세션 쿼터는 계획·오케스트레이션에만 씁니다.
...
| `shared/harness-core.md` | 공통 하네스 정책 — 단일 진실 공급원 |
| `shared/codex-wrapper.md` | Codex 전용 standalone / relay 모드 규칙 |
...
| `scripts/sync-agents.sh` | 공통 core로 `~/.codex/AGENTS.md` 생성 |
| `~/.codex/AGENTS.md` | 자동 생성되는 Codex 단독 실행 하네스 |
...
설치 시 자동으로:
- Claude Code 를 `opusplan` 으로 고정 — Plan 모드만 Opus, 나머지는 Sonnet 오케스트레이션
- Codex 기본 모델을 `gpt-5.5`로 고정 — 최신 코딩 모델
...
- 공통 하네스 core로 `~/.codex/AGENTS.md` 생성 — Codex 단독 폴백용
...
>
> - **Claude Code** — 세션 안에서: `/model opusplan`
>   또는 `~/.claude/settings.json` 에 `"model": "opusplan"` 을 추가·교체.
> - **Codex CLI** — `~/.codex/config.toml` 최상단에 `model = "gpt-5.5"` 추가·교체.
>   한 번만 쓰려면: `codex --model gpt-5.5 ...`
...
대부분의 작업은 Sonnet 으로도 충분합니다. 하지만 시행착오 없이 대규모 작업을 진행하고 싶다면 Plan 모드(`opusplan`) 에서 Opus 를 사용하는 것을 추천합니다.
...
```
 ...
 (Opus)    (GPT-5.5)    (GPT-5.5)      (무료)    (GPT-5.5)
   │           │            │            │           │
...
**Gemini:** 관련 코드나 diff를 읽고 텍스트를 작성합니다 — README 업데이트, 변경 로그, 인라인 주석.
...
2. Codex가 최종 diff를 보고 커밋 메시지를 직접 작성.
3. Codex가 `git add -A && git commit && git push` 실행.

**결과:** `origin`에 깔끔한 커밋 푸시.

**왜 이렇게 나눴나:** Gemini는 다른 단계에서 문서를 맡고, 커밋 메시지는 최종 맥락을 가장 잘 가진 Codex가 직접 씁니다. Claude는 이 단계에 끼지 않습니다.
...
6. **당신:** `/ship`. Codex 테스트 → Codex 커밋 메시지 작성 → Codex 푸시.
...
Claude가 완전히 unavailable이어도 Codex 혼자 전체 흐름을 돌릴 수 있습니다. `~/.codex/AGENTS.md`는 Claude와 같은 `shared/harness-core.md`에서 자동 생성되고, Codex wrapper가 standalone 모드를 켭니다. 계획, 구현, 리뷰, 커밋, 푸시까지 Codex 안에서 이어갑니다.
